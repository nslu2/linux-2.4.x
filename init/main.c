...
//add by super
static struct timer_list sys_led_timer;    /* ide led switch */
void sys_led_blinking(void);
void turn_on_sys_usb(unsigned long ptr)
{
	if (*IXP425_GPIO_GPOUTR&2)
		*IXP425_GPIO_GPOUTR &= ~2;
	else
		*IXP425_GPIO_GPOUTR |= 2;
	sys_led_blinking();
}
...
void sys_led_blinking(void)
{
	init_timer(&sys_led_timer);
	sys_led_timer.function = turn_on_sys_usb;
	sys_led_timer.expires = jiffies + 30;
	add_timer(&sys_led_timer);
}
...
static int init(void * unused)
{
	...
	*IXP425_GPIO_GPOUTR &= 0xfffffffc;
	*IXP425_GPIO_GPOUTR |= 0x2;
	sys_led_blinking();
	//end
	lock_kernel();
	do_basic_setup();
