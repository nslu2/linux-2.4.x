			 ...
			 char *cvf_format, char *cvf_options)
{
	...
	opts->name_check = 'n';
	opts->conversion = 'b';
	opts->fs_uid = 501;//current->uid;
	opts->fs_gid = 501;//current->gid;
	opts->fs_umask = 0007; //current->fs->umask;
...
static void fat_fill_inode(struct inode *inode, struct msdos_dir_entry *de)
{
	...
	if ((de->attr & ATTR_DIR) && !IS_FREE(de->name)) {
		...
		inode->i_mode = (MSDOS_MKMODE(de->attr,
		    ...
		    & ~sbi->options.fs_umask) | S_IFREG) & ~S_IXGRP;
...
retry:
	...
	if (S_ISDIR(inode->i_mode)) {
		...
		raw_entry->size = 0;
		raw_entry->attr |= MSDOS_MKATTR(inode->i_mode & ~S_IXGRP) |
