...
#else
                read_size = min(read_size,cifs_sb->rsize & PAGE_CACHE_MASK);
#endif
		rc = -EAGAIN;
		while(rc == -EAGAIN) {
			if ((open_file->invalidHandle) && (!open_file->closePend)) {
...
cifs_readpage(struct file *file, struct page *page)
{
	...
	if(PAGE_CACHE_SIZE > (unsigned int)rc) {
...
#else
	tmp_inode->i_blocks =
		(tmp_inode->i_blksize - 1 + pfindData->AllocationSize) >> 14;
#endif
...
#else
	tmp_inode->i_blocks =
                (tmp_inode->i_blksize - 1 + pfindData->NumOfBytes) >> 14;
#endif
...
cifs_readdir(struct file *file, void *direntry, filldir_t filldir)
{
	...
	default:
		if (file->private_data == NULL) {
			...
			cFYI(1,("FindNext resume name: %s len: %d",cifsFile->search_resume_name,cifsFile->resume_name_length));
