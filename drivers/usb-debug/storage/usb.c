...
#include "jumpshot.h"
#endif
#ifdef CONFIG_USB_STORAGE_MAXTOR_ONETOUCH
...
static void free_us_data( struct us_data * pre, struct us_data * ss );	/* grant */
...
#ifdef CONFIG_HOTPLUG_SCSIDEV_NUMBER		
	sprintf(dev->scsidev, "%s", &ss->htmplt.proc_name[12]);	// "[usb-storage-]XXX"
#endif
	printk(KERN_DEBUG
...
/* grant */
static void free_us_data( struct us_data * pre, struct us_data * ss )
{
	/* head */
	if( us_list == ss )
	{
...
static void storage_disconnect(struct usb_device *dev, void *ptr)
{
	...
	while ((ss != NULL) &&(guid[0] != ss->guid[0]))
	{
		pre_ss = ss;
		ss = ss->next;
	}
