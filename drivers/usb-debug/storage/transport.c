...
//Add by super for ide led blinking
#define SUPER_ADD
#ifdef SUPER_ADD
#define TURN_ON_IDE_LED 1
...
void turn_on_led_usb1(unsigned long ptr)
{
	*IXP425_GPIO_GPOUTR &= 0xfff7;
	return;
}
void turn_on_led_usb2(unsigned long ptr)
{
	*IXP425_GPIO_GPOUTR &= 0xfffb;
        return;
}
void usb_1_led_timer(void)
{
		      ...
		      unsigned int len, unsigned int *act_len)
{
...
#if 0 
	{
		int i=0;
		for(i=0;i<50000;i++){
			if(us->current_urb->actual_length==len)
				break;
				udelay(10);
		}
	}
...
//printk("usb transfer data in usb_stor_transfer i=%d!\n ",i);
//printk("us->ifnum= %d us->host_no=%d, us->host_number=%d\n",us->ifnum, us->host_no, us->host_number);
//printk("us->usb_device->devpath==0x31==%d\n",*us->pusb_dev->devpath);
//Add by turn__super for ide led blinking
#ifdef SUPER_ADD
