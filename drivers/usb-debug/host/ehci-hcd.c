...
//printk("ehci register address is 0x%x\n",hcd->regs);
	/* EHCI 0.96 and later may have "extended capabilities" */
//printk("ehci pci address is 0x%x\n",ehci->hcd.pdev);
...
done2:
	...
	//Enable test mode by GLI880S 2004/12/21 Start
	for (wIndex = 0; wIndex < 4; wIndex++) {
	    temp = readl (&ehci->regs->port_status [wIndex]);
    	writel (temp | PORT_TESTPKT,
    				&ehci->regs->port_status [wIndex]);
    }				
    //Enable test mode by GLI880S 2004/12/21 End
