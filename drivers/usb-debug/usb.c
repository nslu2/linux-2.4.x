...
static void call_policy_interface (char *verb, struct usb_device *dev, int interface)
{
	...
	envp [i++] = scratch;
	scratch += sprintf (scratch, "SCSIDEV=%s", dev->scsidev) + 1;
#endif
...
void mxo_ot_call_policy (const char *verb, struct usb_device *dev)
{
	call_policy(verb, dev);
}
#endif
...
void usb_disconnect(struct usb_device **pdev)
{
	...
	*pdev = NULL;
	...
	//
	if(dev->speed != 3){
	if ((dev->devnum <= dev_num)||(dev->devnum - dev_num > 100)){
		return;
	}else{
		dev_num = dev->devnum;
	}
