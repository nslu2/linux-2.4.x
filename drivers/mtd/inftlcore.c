 ...
 *
 * $Id: inftlcore.c,v 1.1.1.1 2004/09/28 06:06:11 sure Exp $
 *
...
struct INFTLrecord *INFTLs[MAX_NFTLS];
...
static void INFTL_setup(struct mtd_info *mtd)
{
	...
	DEBUG(MTD_DEBUG_LEVEL3, "INFTL: INFTL_setup(%s)\n", mtd->name);

	for (i = 0; i < MAX_NFTLS; i++) {
		if (!INFTLs[i] && firstfree == -1)
			firstfree = i;
		else if (INFTLs[i] && INFTLs[i]->mtd == mtd) {
			/* This is a Spare Media Header for an INFTL we've already found */
        ...
        if (firstfree == -1) {
		printk(KERN_WARNING "INFTL: no more slots available\n");
		return;
        }

	inftl = kmalloc(sizeof(struct INFTLrecord), GFP_KERNEL);
	if (!inftl) {
		printk(KERN_WARNING "INFTL: no memory for data structures\n");
		return;
	}
	memset(inftl, 0, sizeof(struct INFTLrecord));
...
#ifdef PSYCHO_DEBUG
	printk("INFTL: found new inftl%c\n", firstfree + 'a');
#endif
	...
	/* Partition check ... */
#if LINUX_VERSION_CODE < 0x20328
	resetup_one_dev(&inftl_gendisk, firstfree);
#else
	grok_partitions(&inftl_gendisk, firstfree, 1<<NFTL_PARTN_BITS, inftl->nr_sects);
#endif
}
...
static void INFTL_unsetup(int i)
{
	...
	DEBUG(MTD_DEBUG_LEVEL3, "INFTL: INFTL_unsetup(i=%d)\n", i);
...
{
	DEBUG(MTD_DEBUG_LEVEL3, "INFTL: INFTL_notify_add(%s)\n", mtd->name);
	...
	if (mtd) {
		if (!mtd->read_oob) {
			 ...
			 */
			printk(KERN_WARNING "INFTL: no OOB data, quitting\n");
			return;
		}
...
static void INFTL_notify_remove(struct mtd_info *mtd)
{
	...
	DEBUG(MTD_DEBUG_LEVEL3, "INFTL: INFTL_notify_remove(%s)\n", mtd->name);
...
static int inftl_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	DEBUG(MTD_DEBUG_LEVEL3, "INFTL: inftl_ioctl(cmd=0x%x)\n", cmd);

	inftl = INFTLs[MINOR(inode->i_rdev) >> NFTL_PARTN_BITS];
	...
	case BLKGETSIZE64:
		return put_user(
			(u64)part_table[MINOR(inode->i_rdev)].nr_sects << 9,
			(u64 *)arg);
#endif
...
#if LINUX_VERSION_CODE < 0x20328
		resetup_one_dev(&inftl_gendisk, MINOR(inode->i_rdev) >> NFTL_PARTN_BITS);
#else
		grok_partitions(&inftl_gendisk, MINOR(inode->i_rdev) >> NFTL_PARTN_BITS,
				1<<NFTL_PARTN_BITS, inftl->nr_sects);
#endif
		return 0;
...
void inftl_request(RQFUNC_ARG)
{
	...
	DEBUG(MTD_DEBUG_LEVEL3, "INFTL: inftl_request()\n");
	...
	while (1) {
		...
		DEBUG(MTD_DEBUG_LEVEL3, "INFTL: %s request, from sector "
			"0x%04lx for 0x%04lx sectors\n",
		...
		if (dev >= MAX_NFTLS * (1<<NFTL_PARTN_BITS)) {
			...
			printk(KERN_WARNING "INFTL: bad minor number: "
				"device = %s\n", kdevname(req->rq_dev));
			res = 0; /* fail */
			goto repeat;
		}
		
		inftl = INFTLs[dev / (1<<NFTL_PARTN_BITS)];
		down(&inftl->mutex);

		if (block + nsect > part_table[dev].nr_sects) {
			/* access past the end of device */
			printk(KERN_WARNING "inftl%c%d: bad access: "
				"block = %d, count = %d\n",
		...
		if (req->cmd == READ) {
			...
			for ( ; nsect > 0; nsect-- , block++, buffer += SECTORSIZE) {
				/* Read a single sector to req->buffer + (SECTORSIZE * i) */
				if (INFTL_readblock(inftl, block, buffer)) {
					DEBUG(MTD_DEBUG_LEVEL2, "INFTL: read "
						"request failed\n");
					up(&inftl->mutex);
					res = 0;
					goto repeat;
			...
			for ( ; nsect > 0; nsect-- , block++, buffer += SECTORSIZE) {
				...
				if (INFTL_writeblock(inftl, block, buffer)) {
					DEBUG(MTD_DEBUG_LEVEL1,"INFTL: write "
						"request failed\n");
					up(&inftl->mutex);
					res = 0;
					goto repeat;
...
{
	int inftlnum = MINOR(ip->i_rdev) >> NFTL_PARTN_BITS;
	struct INFTLrecord *thisINFTL;
	thisINFTL = INFTLs[inftlnum];

	DEBUG(MTD_DEBUG_LEVEL3, "INFTL: inftl_open()\n");
...
static int inftl_release(struct inode *inode, struct file *fp)
{
	...
	DEBUG(MTD_DEBUG_LEVEL3, "INFTL: inftl_release()\n");
...
static struct file_operations inftl_fops = {
	read:		block_read,
	write:		block_write,
	ioctl:		inftl_ioctl,
	open:		inftl_open,
...
int __init init_inftl(void)
{
	...
	if (register_blkdev(MAJOR_NR, "inftl", &inftl_fops)){
		printk(KERN_WARNING "INFTL: unable to register block device "
			"on major %d\n", MAJOR_NR);
		return -EBUSY;
	} else {
