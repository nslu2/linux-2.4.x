...
extern int sd_init_onedisk(int i);
		...
		char *scsi_result)
{
	...
	if (!SDpnt->removable)
       {
		...
		buffer = (unsigned char *)kmalloc(512,GFP_KERNEL);
		if (!buffer) {
		...
		do {
			printk("do read capacity!!!\n");
			scsi_cmd[0] = READ_CAPACITY;
			scsi_cmd[1] = 0x0;
			...
			SRpnt->sr_data_direction = SCSI_DATA_READ;
			scsi_wait_req(SRpnt, (void *) scsi_cmd, (void *) buffer,
				    8, SD_TIMEOUT, MAX_RETRIES);
			the_result = SRpnt->sr_result;
			retries--;
	
		} while (the_result && retries);
		printk(" read capacity ok!!!\n");
		capacity = 1 + ((buffer[0] << 24) |
					       (buffer[1] << 16) |
					       (buffer[2] << 8) |
					       buffer[3]);
	...
	SDpnt->host->hostt->removable = SDpnt->removable;
		...
		printk("* %s disk not allowed on port:%c\n", "flash" , SDpnt->host->hostt->port);
		scsi_release_request(SRpnt);
